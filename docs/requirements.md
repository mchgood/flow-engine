...
本文描述需求约束；工程已实现核心流程、Spring/Starter 接入及泛型节点输出。验收项不等于全部验证完成，已验证范围与缺口见 [测试覆盖审查](testing-coverage.md)。未来建议不构成实施授权；与明确用户要求或当前契约存在差异时，应指出具体差异，不重新开启整份需求审批。
...
- 支持矩形多出边/多入边的依赖并行语义，示例推荐显式画并行网关。条件选择必须使用菱形，不能仅给矩形出边加标签。
...
以上决策用于核对后续语义变更。实现结构见 [技术方案](technical-design.md)，接入方式见 [快速使用](quick-start.md)；性能和测试缺口不因工程已实现而视为完成。
